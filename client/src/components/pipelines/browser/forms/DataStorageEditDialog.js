...
export class DataStorageEditDialog extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (!this.isNfsMount) {
          values.sensitiveStorage = this.state.sensitiveStorage;
        }
                  ...
                  <Row>
                    ...
                    <Col xs={24} sm={18}>
                      <Form.Item className={styles.dataStorageFormItem}>
                        <Checkbox
                          disabled={this.props.pending || isReadOnly}
                          onChange={(e) => this.setState({sensitiveStorage: e.target.checked})}
                          checked={this.state.sensitiveStorage}>
                          Sensitive storage
                        </Checkbox>
                      </Form.Item>
                    </Col>
  ...
  checkStorageChanged = (prevProps) => {
    if (!prevProps || prevProps.dataStorage !== this.props.dataStorage) {
      ...
      const sensitiveStorage = this.props.dataStorage
        ? this.props.dataStorage.sensitiveStorage
        : false;
      const sharingEnabled = !this.isNfsMount && this.props.dataStorage
        ? this.props.dataStorage.shared
        : false;
      this.setState({versioningEnabled, sharingEnabled, sensitiveStorage});
    }
  };
